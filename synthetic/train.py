...
parser.add_argument(
    "--model", default="linear", metavar="MODELS", help="linear | cnn | cnn + lstm"
)
parser.add_argument(
    "--gamma",
    type=float,
...
parser.add_argument(
    "--batch-size", type=int, default=256, metavar="B", help="batch size"
)
parser.add_argument(
...
parser.add_argument(
    "--optimizer", default="Adam", metavar="OPT", help="optimizer: Adam | RMSprop"
)
parser.add_argument(
    "--update-freq",
    type=int,
...
# LOG & SAVING
parser.add_argument(
    "--serialize", default=False, action="store_true", help="serialize a model"
)
parser.add_argument(
    "--save",
    default="crl/naive/saved/",
    metavar="SAVE",
    help="path for saving trained models",
)
parser.add_argument(
    "--name", default="", metavar="name", help="specify a name for saving the model"
)
parser.add_argument(
    "--log",
    default="crl/naive/logs/",
    metavar="LOG",
...
def train(env, agent, args):
    ...
    monitor.init_log(
        args.log, "m.{}_e.{}_n.{}".format(args.model, args.env_name, args.name)
    )
    env.reset()
    for num_eps in range(args.episode_num):
        ...
        elif args.env_name == "cartpole1":  # CUSTOM
            # TODO adjust preferences probe
        ...
        while not terminal:
            ...
            if cnt > 100:
                terminal = True
                agent.reset()
            tot_reward = tot_reward + (probe.cpu().numpy().dot(reward)) * np.power(
                args.gamma, cnt
            )
            cnt = cnt + 1
